-- Live GitHub delivers JobAssigned with no runner request ID. The v10 CHECK
-- demanded one for JobAvailable and JobAssigned alike, so every real message
-- was rejected at the durable boundary, never acknowledged, and redelivered
-- forever while the workflow stayed queued.
--
-- JobAvailable keeps the constraint: AcquireJobs is keyed on the request ID, so
-- an available job without one is genuinely unusable. An assignment is stored
-- as evidence only — no durable decision reads its request ID, and pickup is
-- proven by JobStarted/JobCompleted runner identity — so it may be absent.

CREATE TABLE github_message_jobs_v17 (
...
INSERT INTO github_message_jobs_v17(
...
CREATE TABLE github_unpicked_requeue_intents_v17 (
    scale_set_id INTEGER NOT NULL CHECK (scale_set_id > 0),
    runner_request_id INTEGER NOT NULL CHECK (runner_request_id > 0),
    ...
    FOREIGN KEY (scale_set_id, source_message_id, source_event_index)
        REFERENCES github_message_jobs_v17(scale_set_id, message_id, event_index)
        ON DELETE RESTRICT ON UPDATE RESTRICT,
...
INSERT INTO github_unpicked_requeue_intents_v17(
...
ALTER TABLE github_message_jobs_v17 RENAME TO github_message_jobs;
ALTER TABLE github_unpicked_requeue_intents_v17
