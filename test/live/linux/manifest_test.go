...
func writeValidFinalManifest(
	...
	config := liveConfig{
		...
		GitHub: githubConfig{
			ConfigURL:                  "https://github.com/genm/sparerunner-private",
			ClientID:                   "client-id",
			InstallationID:             7,
	...
	writeJSONTestFile(t, config.GitHub.PrivateRepositoryProofFile, privateRepositoryProof{
		...
		Repository: "genm/sparerunner-private",
		Visibility: "PRIVATE",
	}, 0o600)
	...
	if mode == modeCleanupFailure {
		...
		if err := evidence.writeJSON(injectorFileName, injectorEvidence{
			...
			Source: injectorFileEvidence{
				Path: "/opt/sparerunner-test/injector", SHA256: strings.Repeat("9", 64),
				Device: 1, Inode: 2, UID: 0, Mode: 0o500, Size: 100,
			},
			Copy: injectorFileEvidence{
				Path:   "/run/sparerunner-live-injectors/run-test/injector",
...
func validAuthorityEvidence(config liveConfig, generatedAt time.Time) authorityEvidence {
	return authorityEvidence{
		...
		Agent: serviceAuthority{
			...
			MainPID: 101, UID: 1001, ControlGroup: "/system.slice/sparerunner-agent.service",
		...
		Supervisor: serviceAuthority{
			...
			MainPID: 202, UID: 0, ControlGroup: "/system.slice/sparerunner-supervisor.service",
