...
`output/release-evidence/twk-014.json`). The checked-in schema is implemented by
`internal/releaseevidence`; validate it with:

```bash
tewake evidence validate --file output/release-evidence/twk-014.json
```
...
Windows harnesses remain release work for TWK-008 and TWK-009; no local
